...
export function getPullRequestInformation() {
...
export async function formatAndNotify(
  ...
  if (cardLayoutStart === "cozy") {
    webhookBody = formatCozyLayout(commit, conclusion);
  } else {
    webhookBody = formatCompleteLayout(commit, conclusion);
  }
